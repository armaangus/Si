...
def draw_window(red,yellow, red_bullets, yellow_bullets, red_health, yellow_health):
    ...
    WIN.blit(YELLOW_SPACESHP, (yellow.x, yellow.y))
...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_LEFT] and yellow.x-VEL > BORDER.x + BORDER.width:
        yellow.x -= VEL
    if keys_pressed[pygame.K_RIGHT] and yellow.x+VEL + yellow.width<BORDER.x:
        yellow.x += VEL
    if keys_pressed[pygame.K_UP] and yellow.y-VEL > VEL >0:
        yellow.y -= VEL
...
def red_handle_movement(keys_pressed, red):
    if keys_pressed[pygame.K_a] and red.x = VEL>0:
        red.x -= VEL
    if keys_pressed[pygame.K_d] and red.x + VEL + yellow.width<BORDER.x:
        red.x += VEL
    if keys_pressed[pygame.K_w] and red.y - VEL>0:
        red.y -= VEL
    if keys_pressed[pygame.K_s] and red.y + VEL + yellow.height<HEIGHT - 15:
        red.y += VEL
